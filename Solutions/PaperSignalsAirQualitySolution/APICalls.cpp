...
void PaperSignals::AirQuality(String Location){
  ...
  **/
  ...
  // Reset every 49 days. Long variables are 32 bits, so they can store integers up to 4,294,967,295. If we count milliseconds, the variable will overflow every 49 days.  
  // millis() returns the number of milliseconds passed since the ESP8266 board began running the current program.
  if(millis() < lastAQICall) lastAQICall = 0; 
  
  if(updatedIntentTimeStamp || millis() - lastAQICall > timeBetweenAQICalls){
    // TASK B:
    // We only fetch the AQI information if either:
    // 1. We have a new intent parameter. In that case the updatedIntentTimestamp boolean variable will be true. The variable is set in the getSignalByID() function. 
    // or
    // 2. It has been 120 000 milliseconds since we last checked. 
    lastAQICall = millis();
    ...
    // TASK A:
    // Moving the servo to show the AQI (number between 0 and 300). Subtracting from 180 degrees, as the servo moves from 180 on the left, to 0 on the right. 
    int servoPosition = (180-(aqi * 180 / 300));
    Serial.println("Setting servo to" + servoPosition);
    MoveServoToPosition(servoPosition, 10);
  }
